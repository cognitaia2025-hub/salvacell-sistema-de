...
from schemas import (
    ...
    OrderHistoryCreate, OrderHistoryResponse
)
import uuid
import secrets
...
async def create_order(
    ...
    if order_data.device_id:
        device_result = await db.execute(select(Device).where(Device.id == order_data.device_id))
...
@router.post("/{order_id}/history", response_model=OrderHistoryResponse, status_code=status.HTTP_201_CREATED)
